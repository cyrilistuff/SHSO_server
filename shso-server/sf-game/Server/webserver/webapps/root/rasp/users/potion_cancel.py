...
class potion_cancel(HttpServlet): 
	...
	def doPost(self, request, response): 
		...
		category = " " 
		responseBody = "" 
		sql = "select ownable_type_id, SfUserId FROM active_players, active_potion_effects WHERE ShsoUserId="+escapeQuotes(str(shsoID))+";" 
		...
		if ( queryRes == None) or (queryRes.size() == 0): 
			...
			for row in queryRes: 
				...
				if sfsID < 0 : 
					...
					sql = "DELETE FROM active_potion_effects WHERE userid=" + escapeQuotes(str(userID)) + " AND ownable_type_id=" + escapeQuotes(str(ownableID)) + ";" 
		...
		w.println("  <body>") 
		# w.println("  &lt;potion&gt;") 
		w.println("stop_showing_effect%" + str(userID) + "%" + str(ownable_type_id) + "%null") 
