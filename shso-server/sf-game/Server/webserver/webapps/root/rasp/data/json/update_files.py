...
class update_files(HttpServlet):
	...
	def doPost(self, request, response):	
		...
		if ((hashCheckComplete is None or hashCheckComplete == '1') and (username is None or username.lower() not in testers)):
			checkHashes()
			file_bytes_string = ""
			filePaths = ""
			if filePath is None:
				for file in importantFiles.keys():
					...
					filePaths += filePath + ","
				w.print(file_bytes_string + "|" + filePaths)
			elif fileHash is None or fileHash.lower() != importantFiles[filePath].lower():
